...
export function GameModal({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.set("playedAt", String(new Date(`${fd.get("date")}T12:00:00`).getTime()));
    fd.set("result", result);
    fd.set("handQuality", handQ != null ? String(handQ) : "");
    fd.set("prizedCards", JSON.stringify(prized));

    const opp = String(fd.get("opponentDeck") ?? "").trim();
    if (opp && !opponentDecks.includes(opp)) {
      await addCustomOpponentDeckAction(opp);
    }

    setError(null);
    setPending(true);
    const res = mode === "edit" && game ? await updateGameAction(fd) : await addGameAction(fd);
    setPending(false);
    if (res.ok) {
      onClose();
      router.refresh();
  ...
  async function onDelete() {
    ...
    fd.set("id", game.id);
    setPending(true);
    const res = await deleteGameAction(fd);
    setPending(false);
    if (res.ok) {
      onClose();
      router.refresh();

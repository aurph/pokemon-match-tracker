...
const DECK: Array<[number, string, string, string, string]> = [
  // Pokémon (23)
...
function seed() {
  const total = DECK.reduce((s, [c]) => s + c, 0);
  ...
  if (db.select().from(iterations).all().length === 0) {
    const now = Date.now();
  ...
  console.log(`Seeded ${DECK.length} deck-card entries (${total} cards) + v1.0 iteration.`);
}
